...
def sample_txt_file(tmp_path: Path) -> Path:
    """
    Creates a sample .txt file for testing purposes and returns its path.
    """
...
def test_read_txt(sample_txt_file: Path):
    """
    Test the _read_txt method of the Reader class.
    """
...
def test_file_not_found():
    """
    Test if _read_txt function raises a FileNotFoundError when the specified file does not exist.
    """
...
def test_wrong_extension_txt(sample_txt_file: Path):
    """
    Test if _read_txt function raises a ValueError when the file does not have a .txt extension.
    """
